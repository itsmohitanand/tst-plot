...
pairs = collect(combinations(1:5,2))
exp = "attention"

p_list = [L"q_{5}", L"q_{50}", L"q_{95}"]
feature_names = ["P", "T^1", "T^2", "T^3", "LAI"]
...
ax_ice_b = [Axis(f[1,i],
    ...
    ylabel = latexstring("Biomass \\ Loss_{$pft}"),
    xgridvisible = false,
    ygridvisible = false,
    ) for i=1:5]
...
ax_pdp_b1 = [Axis(f[2, i],
    ...
    xgridvisible = false,
    xlabel = latexstring("$(feature_names[pairs[i][1]])"*"_{$(pft)}"),
    ylabel = latexstring("$(feature_names[pairs[i][2]])"*"_{$(pft)}"),
    ygridvisible = false,
    ) for i=1:5]
...
ax_pdp_b2 = [Axis(f[3, i],
    xticks=([1,13,25],  p_list),
    yticks=([1,13,25],  p_list),
    xlabel = latexstring("$(feature_names[pairs[i+5][1]])"*"_{$(pft)}"),
    ylabel = latexstring("$(feature_names[pairs[i+5][2]])"*"_{$(pft)}"),
    xgridvisible = false,
    ygridvisible = false,
    ) for i=1:5]
